# Oisín Ó hÓgaín's Engineering Portfolio

Welcome to my engineering portfolio! I am a passionate and results-driven engineer with expertise in biomedical engineering and 3d modelling. In this repository, you'll find a collection of my engineering projects, research work, and contributions to the field.

Feel free to explore the different sections below to learn more about my background and work.

---

## About Me

I am a ME Biomedical engineering graduate. I have worked on a variety of projects ranging from design, to computer programming and 3d modelling. My skills span across different domains including:

- **Mechanical Engineering**: CAD, material analysis, thermodynamics, Blender, AutoCAD Fusion, SolidWorks
- **Software Engineering**: Python, C++, Java, Matlab
- **Data Science/Analysis**: MATLAB, Excel, R, Minitab

I am passionate about solving complex problems, applying innovative solutions, and continually improving my engineering knowledge. Through this portfolio, I aim to showcase the projects I have worked on and my growth as an engineer.

---
## [Coding Projects](Coding.md) 
## [3D Projects](3DProjects.md) 

### Core Engineering Skills

- **Design & Analysis**: [CAD (SolidWorks, AutoCAD), Structural Analysis, FEA (ANSYS, Abaqus), etc.]
- **Programming**: [Python, MATLAB, C++, Java, JavaScript, etc.]
- **Project Management**: [Agile, Scrum, Microsoft Project, JIRA]
- **Testing & Simulation**: [LabVIEW, Simulink, Circuit Simulation, etc.]
- **Fabrication & Prototyping**: [3D Printing, CNC, PCB Design, etc.]
- **Research**: [Statistical Analysis, Data Collection, Report Writing]

### Tools & Software

- **Engineering Software**: [MATLAB, SolidWorks, ANSYS, AutoCAD, Revit, etc.]
- **Version Control**: [Git, GitHub]
- **Databases**: [SQL, MySQL, MongoDB]
- **Other Tools**: [Simulink, Excel, Power BI, TensorFlow, etc.]

---

---

## Research and Publications

I have also been involved in research and academic projects. Here are a few of my notable works:

- **[Title of Research Paper or Thesis](link-to-paper)** – A detailed research study on [topic], published in [Journal/Conference].
- **[Title of Research Paper or Thesis](link-to-paper)** – Exploring the feasibility of [research topic] in [specific application/field].

[Download Full Papers or Details](link)

---

## Education & Certifications

- **[Your Degree]**, [University Name], [Year]
- **Certification in [Certification Name]**, [Issuing Organization], [Year]
- **Other Training/Courses**:
  - [Course Name], [Platform or Institution]
  - [Course Name], [Platform or Institution]

---

## Contact Me

Feel free to reach out to me for collaboration, consultation, or just to discuss engineering topics!

- **Email**: [youremail@example.com](mailto:youremail@example.com)
- **LinkedIn**: [linkedin.com/in/yourname](https://linkedin.com/in/yourname)
- **Personal Website**: [www.yourwebsite.com](https://www.yourwebsite.com)
- **Twitter**: [@yourhandle](https://twitter.com/yourhandle)

---

